...
static QemuOptsList xenon_opts = {
    ...
    .desc = {
        ...
        {
            .name = "xbox360.storage",
            .type = QEMU_OPT_BOOL,
            .help = "Enable storage (HDD/DVD)",
        },
        {
            .name = "xbox360.region",
            .type = QEMU_OPT_STRING,
...
static void xenon_hardware_reset(void *opaque) {
    ...
    for (int i = 0; i < 3; i++) {
        if (s->cpu[i]) {
            ...
            cpu->env.spr[SPR_HID1] = 0x10000000;   // lol: i misplassed an 1 with an 0 at the start
            cpu->env.spe.spefscr = 0x0;
            if (i == 0) {
                cpu->env.msr |= (1 << MSR_EP);
                cpu->env.msr |= (1 << MSR_IP);
            }
...
static void xenon_machine_init(MachineState *machine) {
    ...
    opts.enable_storage = true;
    QemuOpts *qopts = qemu_opts_find(qemu_find_opts("xbox360-options"), NULL);
    
    if (qopts) {
        const char *val;
        opts.hdd_image = g_strdup((val = qemu_opt_get(qopts, "hdd")) ? val : qemu_opt_get(qopts, "xbox360.hdd"));
        opts.dvd_image = g_strdup((val = qemu_opt_get(qopts, "dvd")) ? val : qemu_opt_get(qopts, "xbox360.dvd"));
        opts.nand_dump = g_strdup((val = qemu_opt_get(qopts, "nand")) ? val : qemu_opt_get(qopts, "xbox360.nand"));
        opts.boot_rom = g_strdup((val = qemu_opt_get(qopts, "bootrom")) ? val : qemu_opt_get(qopts, "xbox360.bootrom"));
        opts.enable_usb = qemu_opt_get_bool(qopts, "usb", qemu_opt_get_bool(qopts, "xbox360.usb", true));
        opts.enable_network = qemu_opt_get_bool(qopts, "net", qemu_opt_get_bool(qopts, "xbox360.net", true));
        opts.enable_storage = qemu_opt_get_bool(qopts, "storage", qemu_opt_get_bool(qopts, "xbox360.storage", true));
        opts.kernel = g_strdup((val = qemu_opt_get(qopts, "kernel")) ? val : qemu_opt_get(qopts, "xbox360.kernel"));
        opts.initrd = g_strdup((val = qemu_opt_get(qopts, "initrd")) ? val : qemu_opt_get(qopts, "xbox360.initrd"));
        opts.append = g_strdup((val = qemu_opt_get(qopts, "append")) ? val : qemu_opt_get(qopts, "xbox360.append"));
    }
    ...
    if (opts.enable_usb) {
        ...
        if (s->usb && s->gic) {
            xenon_usb_connect_irq(s->usb, s->gic, IRQ_USB);
        }
        if (s->usb && s->sata) {
            BlockBackend *hdd = xenon_sata_get_backend(s->sata, SATA_PORT_HDD);
            if (hdd) {
                xenon_usb_attach_msc_backend_lun(s->usb, 0, hdd);
            }
            Error *err = NULL;
            BlockBackend *mu = xenon_create_mu(&err);
            if (!err && mu) {
                xenon_usb_attach_msc_backend_lun(s->usb, 1, mu);
            }
        }
    }

    if (opts.enable_network) {
        s->network = xenon_network_create(get_system_memory(), 0x8000A000);
        printf("[NET]   Initialized at 0x8000A000\n");
        if (s->network && s->gic) {
            xenon_network_connect_irq(s->network, s->gic, IRQ_NETWORK);
        }
    }
