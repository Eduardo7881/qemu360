...
static void xenon_timer_irq_handler(void *opaque, int n, int level) {
    XenonTimerState *s = opaque;
    if (!s->gic) {
        return;
    }
    if (level) {
        xenon_gic_assert_irq(s->gic, IRQ_TIMER0, 0);
    } else {
        xenon_gic_deassert_irq(s->gic, IRQ_TIMER0, 0);
    }
}

static void xenon_timer_individual_irq_handler(void *opaque, int n, int level) {
    XenonTimerState *s = opaque;
    if (!s->gic) {
        return;
    }
    ...
    int irq = base + n;
    if (level) {
        xenon_gic_assert_irq(s->gic, irq, 0);
    } else {
        xenon_gic_deassert_irq(s->gic, irq, 0);
    }
}
